...
def rsnet18(pretrained=False):
    model = RSNet(BasicBlock, [2, 2, 2, 2])
    if pretrained:
...
def rsnet50(pretrained=False, num_class=45):
    model = RSNet(Bottleneck, [3, 4, 6, 3], num_classes=num_class)
    if pretrained:
        features = models.resnet50(pretrained=True)
        model.conv1 = features.conv1
        model.bn1 = features.bn1
        model.relu = features.relu
        model.maxpool = features.maxpool
